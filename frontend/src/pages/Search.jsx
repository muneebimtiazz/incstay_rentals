...
const Search = () => {
  ...
  useEffect(() => {
    const fetchRents = async () => {
      try {
        const pre = await axios.get('/api/predefinedRents/filter', {
        params: {
          city: city || null,
          maxGuest: maxGuest || null
        }
      });
      setRent(pre.data)
        const user = await axios.get('/api/userdefinedRents/filter', {
        params: {
          city: city || null,
          maxGuest: maxGuest || null
        }
      });
        setUserRent(user.data)
      } catch (err) {
      ...
      <div className='grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-col-5 gap-6'>
         ...
         return <NavLink to={`/info/${item._id}`} key={item._id} className='cursor-pointer group transition-all duration-300 hover:shadow-lg overflow-hidden flex flex-col h-full p-2 border rounded border-gray-200 space-y-1 '>
            ...
            <div className='overflow-hidden relative aspect-[4/3]'>
              ...
              {!item.reviews&&<span className='text-xs rounded absolute top-2 left-2 px-2 py-1 text-white bg-red-500'>Recently Added</span>}
            </div>
                    ...
                    <div className='flex items-center gap-1'>
                        <IoStarOutline />
                        {item.rating && item.reviews?<p>{item.rating} ({item.reviews})</p>
                                    :<p>No reviews yet</p>
                        }
                    </div>
        ...
        {rent.map((item)=>{
         return <NavLink to={`/info/${item._id}`} key={item._id} className='cursor-pointer group transition-all duration-300 hover:shadow-lg overflow-hidden flex flex-col h-full p-2 border rounded border-gray-200 space-y-1 '>
            ...
            <div className='overflow-hidden relative aspect-[4/3]'>
              <img className='absolute top-0 left-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-105 rounded' src={item.images[0]} alt="image" />
              {item.guestFavoriate&&<span className='text-xs rounded absolute top-2 left-2 px-2 py-1 bg-gray-100'>Guest favoriate</span>}
            </div>
